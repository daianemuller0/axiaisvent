...
namespace HowdenAxiais.Launcher;
...
///  - na REDE (\\...\VA\): VA.exe + app\versao.txt + app\&lt;versão&gt;\ (imutável);
///  - na MÁQUINA (%LOCALAPPDATA%\HowdenVA\): cópia local do app E do próprio
///    VA.exe — na primeira abertura pela rede, o lançador SE INSTALA aqui e
...
{
    private const string ExeDoApp = "HowdenAxiais.exe";
    private const string NomeAtalho = "VA - Ventiladores Axiais Howden.lnk";
    ...
    private async Task ExecutarAsync()
    {
        try
        {
            ...
            var raizLocal = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "HowdenVA");
            Directory.CreateDirectory(raizLocal);
            var svLocal = Path.Combine(raizLocal, "VA.exe");
            ...
            {
                var svRede = Path.Combine(raizRede, "VA.exe");
                try
                {
            ...
            if (string.IsNullOrWhiteSpace(versao))
            {
                ...
                if (ultima is null)
                    throw new Exception(
                        "Sem acesso à pasta de rede do VA e nenhuma versão instalada nesta máquina ainda.\n" +
        ...
        {
            MessageBox.Show(this, "Não foi possível abrir o VA:\n\n" + ex.Message,
                "VA", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        Close();
    }

    /// <summary>Atalho "VA - Ventiladores Axiais Howden" na área de trabalho, apontando para o VA.exe LOCAL.</summary>
