# =====================================================================
# Publica o VA na pasta de rede, em versao nova (ninguem trava arquivo):
#
#   \\BZVCPFIL003\proj_ramires$\VA\
#     VA.exe               <- lancador (atalho dos usuarios aponta pra ele)
#     app\versao.txt       <- nome da versao atual (ex.: v2026-09-09_1530)
#     app\v2026-09-09_1530\  <- arquivos publicados desta versao
#
# (a base de dados continua em \\BZVCPFIL003\proj_ramires$\DB\axiais,
...
# =====================================================================
param(
    [string]$Destino = '\\BZVCPFIL003\proj_ramires$\VA',
    [int]$ManterVersoes = 5
)
...
if (-not (Test-Path "$raiz\desktop\HowdenAxiais.Desktop.csproj")) {
    Stop-Transcript | Out-Null
    throw "A pasta 'desktop' nao existe aqui - rode 'git pull' para baixar a versao mais nova do projeto e tente de novo."
}

Write-Host "== VA: publicando versao $versao em $Destino ==" -ForegroundColor Cyan
...
dotnet publish "$raiz\desktop\HowdenAxiais.Desktop.csproj" -c Release -r win-x64 `
...
dotnet publish "$raiz\launcher\HowdenAxiais.Launcher.csproj" -c Release -r win-x64 `
...
try {
    Copy-Item "$raiz\out\launcher\VA.exe" "$Destino\VA.exe" -Force
} catch {
    Write-Warning "Nao consegui atualizar o VA.exe (alguem deve estar abrindo o app agora). Rode de novo mais tarde se o lancador tiver mudado."
}
...
Write-Host "Atalho para os usuarios: $Destino\VA.exe"
